...
1. **右侧边缘常驻入口(必现,以此为准)**:浏览器窗口**右侧边缘、垂直居中**处有一枚小标签「**● 记忆 N**」。它由插件以 **body 级 DOM 挂载**,`z-index` 高于所有面板,点开后在**右侧顶层浮层抽屉**(浏览器 top-layer)里显示面板,任何面板都盖不住它。
2. **会话标题栏入口(自动回退)**:若运行环境无法挂载 body 级入口(例如 `react-dom` 不可用),自动回退到会话标题栏右上角(与「下载会话日志」图标并排),面板仍在 top-layer。
3. **右侧工作台 tab(彩蛋)**:若部署里安装了 `dsh-better-sidebar` 且与本插件处于**同一个客户端组装域**,工作台的「+ / 空面板卡片」里还会多出一项「**Memorix 记忆**」。

> ⚠️ **为什么入口 1 用 body 级挂载**:DSH 的浏览器侧由**多个客户端组装域**组成,而槽位系统(`slots`)与 `dsh-better-sidebar` 的 tab 注册表都是**按域隔离**的:插件实例可能被挂进一个不被可见 UI 读取的域,于是出现"注册成功但界面上看不到"的静默失败(实测踩过)。body 级挂载不依赖任何域,是唯一能保证"必现"的方式。
...
硬刷新页面后**不需要打开会话**也能看到入口:

- 先看**浏览器窗口右侧边缘、垂直居中**处的标签「**● 记忆 N**」——这是判断插件是否生效的依据;
- 点它 → 右侧滑出顶层浮层抽屉(面板顶部显示记忆库路径与大小);
- 若你在会话中,标题栏右上角(「下载会话日志」旁)也可能出现同样的入口(回退路径);
- 若你装了 `dsh-better-sidebar` 且与插件同域,工作台「+ / 空面板卡片」里还会多一项「Memorix 记忆」(可选,不出现也不影响使用)。
