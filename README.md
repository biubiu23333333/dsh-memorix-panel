...
1. **会话标题栏入口(必挂,以此为准)**:会话标题栏右上角的原生按钮「**● 记忆 N**」——和「下载会话日志」图标并排。点击后在**右侧顶层浮层抽屉**(浏览器 top-layer)里打开面板,不会被任何其它面板遮挡。
2. **右侧工作台 tab(尽力而为)**:若部署里同时安装了 `dsh-better-sidebar`,插件还会尝试注册一个工作台 tab「**Memorix 记忆**」,出现在「+ / 空面板卡片」列表里。

> ⚠️ 关于入口 2:DSH 的浏览器侧由**多个客户端组装域**构成,而 `dsh-better-sidebar` 的 tab 注册表按域隔离。只有当本插件与工作台 UI 落在**同一个域**时该 tab 才会出现;不同 DSH 版本/部署的域结构可能不同,因此**请以入口 1 为准**,入口 2 只是额外便利。
...
- **会话标题栏右上角**应出现原生按钮「**● 记忆 N**」(在「下载会话日志」图标旁),点击后弹出右侧顶层浮层抽屉 —— 这是判断插件是否生效的依据;
- 若你装了 `dsh-better-sidebar` 且它与本插件处于同一客户端组装域,右侧工作台的「+ / 空面板卡片」里还会多出一项「**Memorix 记忆**」(可选,不出现也不影响使用)。

面板顶部应显示记忆库路径与大小;统计区应给出记忆/项目/活跃/归档等数量。如果数量为 0,先按第一步第 5 小节在一个真实项目里跑一次 `memorix status`。
