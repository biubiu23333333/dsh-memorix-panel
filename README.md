...
插件启动时按当前部署**二选一**挂载入口,不会出现两个重复入口:

1. **装了 `dsh-better-sidebar`**(推荐):在**右侧工作台注册一个 tab「Memorix 记忆」**——入口就在右侧工作区里,和 Files / 文件变动 / 终端 / 浏览器 并列。
2. **没装 `dsh-better-sidebar`**:退化到**会话标题栏右上角的原生按钮「● 记忆 N」**,点击后在**右侧顶层浮层抽屉**(浏览器 top-layer)里打开面板,不会被其它面板遮挡。
...
硬刷新页面后,打开任意一个会话(按你的部署二选一):

- **装了 `dsh-better-sidebar`**:右侧工作台的标签栏里应多出一个 tab「**Memorix 记忆**」;
- **没装 `dsh-better-sidebar`**:会话标题栏右上角应出现原生按钮「**● 记忆 N**」,点击后弹出右侧顶层浮层抽屉。
