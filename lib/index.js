...
async function readSnapshot(config) {
  try {
    ...
    try {
      ...
      const error = new Error('无法读取 Memorix 存储(需要 Node >= 22.5 或 python3)')
      error.detail = detail
      throw error
    }
  }
...
/** Human-readable "how long ago" for an ISO timestamp. */
function ageOf(iso, now) {
  ...
  if (seconds < 3600) return String(Math.floor(seconds / 60)) + ' 分钟前'
  if (seconds < 86400) return String(Math.floor(seconds / 3600)) + ' 小时前'
  return String(Math.floor(seconds / 86400)) + ' 天前'
}
...
async function readLongTermTables(config) {
  try {
    ...
    try {
      ...
      const error = new Error('无法读取长期记忆(需要 Node >= 22.5 或 python3)')
...
async function ltTransition(config, payload) {
  if (!config.allowWrite) return { ok: false, error: '该部署已禁用写入(allowWrite: false)' }
  const id = typeof payload?.id === 'string' ? payload.id.trim() : ''
  if (!/^[A-Za-z0-9-]{8,80}$/.test(id)) return { ok: false, error: '无效的长期记忆 id' }
  const verb = typeof payload?.verb === 'string' ? payload.verb : ''
  if (!LONG_TERM_ACTIONS.includes(verb)) {
    return { ok: false, error: '无效的动作(仅允许 qualify / approve / archive)' }
  }
  ...
  if (root === undefined) {
    return { ok: false, error: '未找到项目根路径', detail: '缺少 .project-aliases.json 与 last-project-root' }
  }
  const result = await runMemorix(config, [
    'memory', 'long-term', verb, '--id', id, '--reason', reason, '--json', '--cwd', root,
  ])
  if (!result.ok) {
    return { ok: false, error: result.parsed?.error ?? '长期记忆操作失败', detail: result.stderr }
  }
  const state = result.parsed?.memory?.state
  if (typeof state !== 'string' || state === '') {
    return { ok: false, error: 'CLI 未返回长期记忆状态', detail: JSON.stringify(result.parsed ?? {}).slice(0, 400) }
  }
...
async function loadOverview(config) {
  ...
  const projects = snapshot.projects.map((project) => ({
    ...
    lastAge: ageOf(project.lastAt, now),
    types: project.types,
    memories: project.memories.map((memory) => ({
      ...memory,
      age: ageOf(memory.createdAt, now),
    })),
  }))
...
async function readMemoryRow(config, id) {
  ...
  if (!record) return { ok: false, error: '未找到编号 #' + String(id) + ' 的记忆' }
...
async function setStatus(config, payload) {
  if (!config.allowWrite) return { ok: false, error: '该部署已禁用状态写入(allowWrite: false)' }
  const id = parseId(payload?.id)
  if (id === null) return { ok: false, error: '无效的记忆编号' }
  const status = typeof payload?.status === 'string' ? payload.status : ''
  if (!STATUSES.includes(status)) {
    return { ok: false, error: '无效的状态(仅允许 active / resolved / archived)' }
  }
  const { projectId, root } = await rootForMemory(config, id)
  if (root === undefined) {
    return { ok: false, error: '未找到该记忆所属项目的根路径', detail: '缺少 .project-aliases.json 与 last-project-root' }
  }
  const result = await runMemorix(config, ['memory', 'resolve', '--id', String(id), '--status', status, '--json', '--cwd', root])
  if (!result.ok) {
    return { ok: false, error: result.parsed?.error ?? '写入 Memorix 失败', detail: result.stderr }
  }
  ...
  if (!applied.some((value) => Number(value) === id)) {
    return { ok: false, error: 'CLI 未更新该记忆', detail: '项目: ' + root }
  }
...
async function storeMemory(config, payload) {
  if (!config.allowStore) return { ok: false, error: '该部署已禁用新建记忆(allowStore: false)' }
  ...
  if (text.trim() === '') return { ok: false, error: '正文不能为空' }
  ...
  if (root === undefined) {
    return {
      ok: false,
      error: '未找到写入目标项目的根路径',
      detail: wanted === '' ? '缺少 last-project-root' : '项目 ' + wanted + ' 不在 .project-aliases.json 中',
    }
  }
  ...
  if (!observation?.id) {
    return { ok: false, error: 'CLI 未返回新建的记忆', detail: JSON.stringify(result.parsed).slice(0, 400) }
  }
  return {
    ok: true,
    id: observation.id,
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        writeJson(res, 400, { ok: false, error: '请求体解析失败', detail: describe(error) })
        return
      }
      ...
      try {
        ...
        switch (action) {
          ...
          case 'memory': {
            const id = parseId(payload?.id)
            result = id === null ? { ok: false, error: '无效的记忆编号' } : await loadMemory(resolved, id)
            break
          }
          case 'set-status':
          ...
          case 'store':
            ...
            break
          default:
            result = { ok: false, error: '未知操作: ' + action }
        }
