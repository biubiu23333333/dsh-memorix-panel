...
`
    ...
    function MemorixPanel(props) {
      ...
      const head = h('div', { className: 'mxov-head', key: 'h' }, [
        ...
        variant === 'floating' ? h('button', { className: 'mxov-btn', key: 'c', onClick: () => { setOpen(false); setOpenId(null); setComposing(false) }, title: '关闭' }, '关闭') : null,
      ])
    ...
    function apply(ctx) {
      ...
      // Guaranteed seat: the shipped session-header utilities. This is the one
      // every DSH web composition renders, and its panel opens in the browser
      // top layer so no other panel can cover it.
      ...
      // Best-effort seat: a right-workbench tab in dsh-better-sidebar. Only
      // lands when this plugin instance shares the workbench's service
      // instance (same client composition), so it is never relied upon.
      ...
      // One line of startup self-report: which seat this instance took.
      console.info('[dsh-memorix-panel] seat', 'entry=' + String(mountedEntry), 'tab=' + String(mountedTab))
    }
